...
pajhwokApp.config(['$routeProvider',
    function ($routeProvider) {
        $routeProvider.
            ...
            }).when('/map/category/:parentCatId/subcategory/:childCatId/language/:languageid/search', {
                templateUrl:'../mapview',
                controller:'MapCtrl'
            }).when('/search/category/:parentCatId/subcategory/:childCatId/state/:stateid/language/:languageid', {
                templateUrl:'../search',
...
//        mapTypeId: google.maps.MapTypeId.ROADMAP
    ...
    $scope.managemarker = function () {
        ...
        query.columns = [
            {"expression":"_id", "aggregate":"count", "alias":"count"}
        ];
        query.filter = { "__createdon":"last-3-days","status":"approved"};
...
pajhwokApp.controller('searchCtrl', function ($scope, $compile, $http, $location, $appService, $rootScope, $routeParams) {
    ...
    $scope.getSearchNews = function (category, subcategory, state, language, user, search, max_row, cursor) {
        $scope.searchView = true;
        $scope.loadingMessage = true;
        ...
        $appService.getDataFromJQuery(serviceURL, queryParams, "GET", "JSON", function (recentStoriesData) {
            ...
            for (var i = 0; i <  recentStoriesData.response.data.length; i++) {
                recentStoriesData.response.data[i].description = ( recentStoriesData.response.data[i].description) ? unescape( recentStoriesData.response.data[i].description) : ""
            }
            $scope.recent.totalCount = recentStoriesData.response.$count;
            $scope.searchedNews = $scope.setUrls( recentStoriesData.response.data,580);
    ...
    $scope.getMoreSearch= function (category, subcategory, state, language, user, search, max_row, cursor) {
        ...
        $appService.getDataFromJQuery(serviceURL, queryParams, "GET", "JSON", function (recentStoriesData) {
            ...
            console.log("before"+JSON.stringify($scope.searchedNews[0]));
            for (var i = 0; i < recentStoriesData.response.data.length; i++) {
                recentStoriesData.response.data[i].description = (recentStoriesData.response.data[i].description) ? unescape(recentStoriesData.response.data[i].description) : ""
            }
            recentStoriesData.response.data = $scope.setUrls(recentStoriesData.response.data,580);
            for (var i = 0; i < recentStoriesData.response.data.length; i++) {
                $scope.searchedNews.push(recentStoriesData.response.data[i]);
                console.log($scope.searchedNews.length);
            }
            console.log("after"+JSON.stringify($scope.searchedNews[$scope.searchedNews.length-1]));
            if (!$scope.$$phase) {
                $scope.$apply();
            }
    ...
    $scope.getMoreSearchCall=function(){
        $scope.getMoreSearch($routeParams.parentCatId,$routeParams.childCatId, $routeParams.stateid, $routeParams.languageid);
    }
    $scope.getSearchNews($routeParams.parentCatId,$routeParams.childCatId, $routeParams.stateid, $routeParams.languageid);
});
pajhwokApp.controller('articleCtrl', function ($scope, $compile, $http, $location, $appService, $rootScope, $routeParams,$window) {
    ...
    $scope.getArticleNews = function () {
        ...
        if (true) {
            ...
            query.columns = ["_id", "title", "media","guest.name", "category", "status", "likeby", "likecount", "flaggedby", "description", "commentcount", "clickcount", {"expression":"comment", "columns":["content", "__createdby.firstname", {"expression":"__createdon", "format":"fromnow"}]}, "image", "__createdby.firstname", "activitycount", "clickcount", {"expression":"__createdon", "format":"fromnow"}];
            ...
            $appService.getDataFromJQuery(serviceURL, queryParams, "GET", "JSON", function (topStoriesData) {
                $scope.news = $scope.setUrls(topStoriesData.response.data,580);
                $scope.newsVisible = "";
...
pajhwokApp.controller('homeCtrl', function ($scope, $compile, $http, $location, $appService, $rootScope,$window) {
...
//            for (var i = $scope.recent.cursor; i < recentStoriesData.response.data.length + $scope.recent.cursor; i++) {
//                if($scope.recentStoriesDetail[i].media && $scope.recentStoriesDetail[i].media.length){
//                    $scope.recentStoriesDetail[i]["displayType"]={"image":false,"audioVideo":true};
//                    $scope.recentStoriesDetail[i]["fileUrl"]= BAAS_SERVER + "/file/render?filekey=" + $scope.recentStoriesDetail[i]["media"][0]["key"] + '&ask=' + ASK + '&osk=' + OSK;
//                    if($scope.recentStoriesDetail[i]["media"][0]["capturedimagekey"]) {
//                        $scope.recentStoriesDetail[i]["thumbnail"]=BAAS_SERVER + "/file/render?filekey=" + $scope.recentStoriesDetail[i]["media"][0]["capturedimagekey"] + '&ask=' + ASK + '&osk=' + OSK;
//                    }
//                    else{
//                        $scope.recentStoriesDetail[i]["thumbnail"] = "/rest/file/render?filekey=530835834b43556925e9b1ee&ask=" + ASK + "&osk=" + OSK + '&resize={"width":253,"height":180}&convert={"type":"png"}';
//                    }
//
//                }
//                else if ($scope.recentStoriesDetail[i]["image"]) {
//                    $scope.recentStoriesDetail[i]["displayType"]={"image":true,"audioVideo":false};
...
pajhwokApp.controller('pajhwokCtrl', function ($scope, $compile, $http, $location, $appService, $rootScope) {
    ...
    $scope.getDefaultMedia = function () {
        ...
        $appService.getDataFromJQuery("/rest/data", params, "GET", "JSON", function (callBackData) {
            ...
            console.log("default media" + JSON.stringify($scope.defaultMediaVal));
    ...
    $scope.setUrls = function (data,size) {
...
//        }
    ...
    $scope.getCategories = function () {
        ...
        query.childs = [{"alias":"childs","usein":true,"relatedcolumn":"parentcategoryid","query":{"table":"Category__pajhwok","columns":["id","en_title","ps_title","dr_title"]}}];
    ...
    $scope.getStates = function () {
        ...
        $appService.getDataFromJQuery("/rest/data", params, "GET", "JSON", function (callBackData) {
            ...
            $scope.states.splice(0, 0, {"name":"All", "ps_name":"ټول", "dr_name":"همه", "_id":null});
...
//        query.cursor = $scope.recent["cursor"];
        ...
        $appService.getDataFromJQuery(serviceURL, queryParams, "GET", "JSON", function (recentStoriesData) {
            ...
            $scope.recent.cursor = recentStoriesData.response.cursor;
            $scope.recent.totalCount = recentStoriesData.response.$count;
            $scope.recentStoriesDetail = $scope.setUrls(recentStoriesData.response.data,580);
...
//        $scope.displayType.imageFileUrl='images/loading18.gif';
    ...
    $scope.setCategory = function (categoryData) {
        ...
        if (condition) {
            ...
            $scope.uploadNews = false;
            window.location.href = '#/map/category/' + ($scope.selectedCategoryid && $scope.selectedCategoryid.parent ? $scope.selectedCategoryid["parent"] : "all") + '/subcategory/all/language/' + $scope.selectedTrans + '/search?q=' + null;
            ...
            $scope.uploadNews = false;
            window.location.href = "#/search/category/" + ($scope.selectedCategoryid && $scope.selectedCategoryid.parent ? $scope.selectedCategoryid["parent"] : "all") + "/subcategory/all/state/" + $scope.defaultSelected.selectedState._id + "/language/" + $scope.selectedTrans;
        }
    };
    ...
    $scope.setSubCategory = function (subCategoryData) {
        ...
        if (condition) {
            ...
            $scope.uploadNews = false;
            window.location.href = '#/map/category/' + $scope.selectedCategoryid["parent"] + "/subcategory/" + $scope.selectedCategoryid["child"] + '/language/' + $scope.selectedTrans + '/search?q=' + null;
            ...
            $scope.uploadNews = false;
            window.location.href = "#/search/category/" + $scope.selectedCategoryid["parent"] + "/subcategory/" + $scope.selectedCategoryid["child"] + "/state/" + $scope.defaultSelected.selectedState._id + "/language/" + $scope.selectedTrans;
        }
    };
